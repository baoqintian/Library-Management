...
public class LibraryDao {
    ...
    private QueryRunner queryRunner = new TxQueryRunner();////接收一个连接池对象方便其底层创建Connection对象
    ...
    public void add(Library library){
        try{
            ...
            Object[] parameter = {library.getId(),library.getName(),library.getAuthor(),library.getDate(),
                    library.getPrice(), library.getInventory()};
    ...
    public void edit(Library library){
        try{
            String sql = "UPDATE library SET  name = ?, author = ?, date = ?, " +
                    "price = ?, inventory = ? WHERE id = ?";
            Object[] parameter = {library.getName(),  library.getAuthor(), library.getDate(),
                    library.getPrice(), library.getInventory(), library.getId()};
            queryRunner.update(sql,parameter);
    ...
    public Library find(String id){
        try{
            ...
            return queryRunner.query(sql, new BeanHandler<>(Library.class),id);
        }catch (SQLException e){
            e.printStackTrace();
    ...
    public PageBean<Library> showAll(int pageCode, int pageRecord){
        try{
            PageBean<Library> pageBean = new PageBean<>();
            ...
            //检索从第?行到第?行的记录
            sql = "SELECT * FROM library ORDER BY name LIMIT ?,?";
            Object[] parameter = {(pageCode - 1) * pageRecord, pageRecord};     //数组从0开始，而页码从1开始

            List<Library> beanList = queryRunner.query(sql, new BeanListHandler<>(Library.class),
                    parameter);
    ...
    public PageBean<Library> query(Library library, int pageCode, int pageRecord) throws UnsupportedEncodingException {
        ...
        try{
            ...
            StringBuilder countSQL = new StringBuilder("SELECT COUNT(*) FROM library ");
            ...
            if(author != null && !author.trim().isEmpty()){
                ...
                parameter.add(author);
            }


            String inventory = library.getInventory();
            ...
            //先查询出有多少条记录
            Number totalRecord =(Number) queryRunner.query(countSQL.append(whereSQL).toString(), new ScalarHandler<>(),parameter.toArray());
            pageBean.setTotalRecord(totalRecord.intValue());

            //根据查询出的记录数量，在此基础上，限制内容，分页输出
            StringBuilder selectSQL = new StringBuilder("SELECT * FROM library ");
            ...
            //以List的形式，将查询到的数据封装
            List<Library> beanList = queryRunner.query(selectSQL.append(whereSQL).append(limitSQL).toString(),
                    new BeanListHandler<>(Library.class), parameter.toArray());
            //为pageBean设定页面内容
            pageBean.setBeanList(beanList);
            return pageBean;
